...
#
# $Id: Mysql.pm,v 1.16 2004/07/19 19:50:21 brondsem Exp $
...
$CVSID = '$Id: Mysql.pm,v 1.16 2004/07/19 19:50:21 brondsem Exp $ ';
...
sub new {
	...
	$self->{files_insert} =
	  $self->{dbh}->prepare("insert into files (filename, revision, fileid) values (?, ?, NULL)");
	...
	$self->{symbols_insert} =
	  $self->{dbh}->prepare("insert into symbols (symname, symid) values ( ?, NULL)");
	...
	$self->{releases_insert} =
	  $self->{dbh}->prepare("insert into releases (fileid, release) values (?, ?)");
	...
	$self->{usage_insert} =
	  $self->{dbh}->prepare("insert into useage (fileid, line, symid) values (?, ?, ?)");
	$self->{usage_select} =
	...
	$self->{decl_select} =
	  $self->{dbh}
	  ->prepare( "select declid from declarations where langid = ? and " . "declaration = ?" );
	$self->{decl_insert} =
	  $self->{dbh}
	  ->prepare("insert into declarations (declid, langid, declaration) values (NULL, ?, ?)");
	...
	$self->{delete_indexes} =
	  $self->{dbh}->prepare( "delete from indexes "
		  ...
		  . "and r.release = ?" );
	$self->{delete_useage} =
	  $self->{dbh}->prepare( "delete from useage "
